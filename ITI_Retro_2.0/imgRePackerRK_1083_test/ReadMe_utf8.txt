[ 26.06.2025 ]=====================================================================================
		...
		version 1.0.8.3 (windows&linux)
		(c) RedScorpio, Moscow, 2013-2025
...
Также можно использовать для исправления контрольных сумм md5 и RockChip CRC. 
    ...
    /rkbfs	- использовать значение FileSpace для RK_bootloader при запаковке (rkbfs = 0);
    ...
    Примечание:
    Для распаковки/запаковки gzip/cpio файлов Windows-версия утилиты использует внешнюю библиотеку
...
History:
1.0.8.3 [W&L] (26.06.2025)
	~ изменения в связанных библиотеках. 
1.0.8.2 [W&L] (28.04.2025)
	~ изменения в связанных библиотеках. 
1.0.8.1 [W&L] (17.03.2025)
	~ мелкие улучшения. 
1.0.8.0 [W&L] (26.11.2024)
	~ изменения в системе логгирования. 
1.0.7.7 [W&L] (08.04.2024)
	~ мелкие улучшения. 
1.0.7.6 [W&L] (07.09.2023)
	~ мелкие улучшения. 
1.0.7.5 [W&L] (25.08.2023)
 	+ распаковка форматов android boot v.3 и v.4;
	~ мелкие улучшения. 
1.0.7.4 [W&L] (21.04.2023)
	~ мелкие улучшения. 
1.0.7.3 [W&L] (??.02.2023)
 	~ изменен алгоритм распаковки/упаковки формата rk_uboot;
	~ мелкие улучшения. 
1.0.7.2 [W&L] (30.12.2022)
 	+ улучшена распаковка/упаковка формата android boot v.0;
 	+ распаковка/упаковка формата android boot v.1;
	+ обработка разных для RKFW и RKAF loader-ов;
	+ sparse images packing added;
	+ SHA256 for rk_uboot added;
	+ исправлена распаковка больших рамдисков (gzcpio/cpio);
	+ исправлено определение версии при распаковке (если версия в виде "7.0");
	- устранен креш при запуске с неправильными параметрами; 
	+ добавлен ключ /rkbfs;
	+ реализована обработка RKAF нового формата (количество файлов в прошивке > 16);
	~ мелкие улучшения. 
1.06 [W&L] (03.05.2017)
	+ добавлена поддержка формата RockChip uboot;
	+ добавлен метод DirtyBlk;
	+ поддержка старого формата RKFW;
	+ поддержка RK28xx (ChipID=0xXXXXXX2X);
	+ поддержка RK33xx (ChipID=0xXXXXXX4X);
...
1.05 [W&L] (10.08.2015)
	...
	+ поддержка RK32xx (ChipID=0xXXXXXX8X);
	+ добавлен ключ /symb;
